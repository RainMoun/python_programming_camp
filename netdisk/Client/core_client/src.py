from interface_client import admin_interface, common_interface, user_interface
...
def register():
    ...
    while True:
        ...
        if password == conf_password:
            flag, msg, now_user = common_interface.register_interface(name, password, user_data['status'])
            if flag:
                user_data["name"] = now_user
                print(msg)
                break
...
def upload_file():
    ...
    if user_data['status'] == 1:
        ...
        flag, msg = user_interface.upload_file_interface(user_data['name'], file_path)
    print(msg)


@login_auth
def download_file():
    flag, msg = user_interface.download_file_interface(user_data['name'])
    print(msg)
...
def join_membership():
    ...
    if input_confirm == 'n':
        ...
        user_data['name'].is_member = 1
        msg = user_interface.join_membership_interface(user_data['name'])
        print(msg)
...
user_func_dict = {'1': {'fun': register, 'explain': '注册'},
                  '2': {'fun': login, 'explain': '登录'},
                  '3': {'fun': upload_file, 'explain': '上传文件'},
                  '4': {'fun': download_file, 'explain': '下载文件'},
                  '5': {'fun': join_membership, 'explain': '开通会员'},
